				...
				<section id="jcDayLovePub" class="pub-user">
					<div class="user-list">
						<ul class="clearfix" id="qingYuanRecommend">
							<c:forEach items="${lists }" var="userpp">
								<li id="${userpp.uid }" class="showFlag fadeFromLeft"><div
										class="user-item front clearfix">
										<div class="photo">
											<a><img src="<%=basePath%>${userpp.picture.paddress }"></a>
										</div>
										<div class="user-info">
											<h3>
												<a class="name">${userpp.name }</a><a class="flag-zx"></a> <a
													class="flag-mobile"></a><a class="flag-iden"></a> <a
													class="flag-credit credit-js">满分</a>
											</h3>
											<p class="base">${userpp.age },${userpp.address },</p>
...
<%-- 						<c:forEach begin="1" end="${allpage } " step="1" var="pagenum"> --%>
<%-- 						<a href="<%=basePath %>page=${pagenum}"><div class="mybtn">${pagenum}</div></a> --%>
<%-- 						</c:forEach> --%>
		...
		<div class="frame-right">
			<div class="mod-block-3 exp-preson-info showFlag fadeFromBottom"
				...
				<div class="exp-p-info">
					...
					<div class="user-100">
						<a target="_blank"><img
							src="<%=basePath%>${userP.picture.paddress }"></a>
					</div>
					<dl>
						<dt>
							<a class="myname" href="" target="_blank">${userP.name }</a> <a
								class="flag-zx-n"
...
//标识id:
	...
	div3.onclick = function() {
		location.href="<%=basePath%>jsp/person.jsp";
	}
	// 同城匹配的点击事件
...
// 		});
     /*  document.cookie="uid=${list2[0].uid}",path="/";
      ...
      window.location="<%=basePath%>home_select";
	};
 //异城匹配
	var div3 = document.querySelector("#search2");
	div3.onclick=function(){
		
		location.href="<%=basePath%>home_select2";
	}
